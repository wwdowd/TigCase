...
//tig_cycle.csv file on SD card has 2 columns w/ pH,DO setpoints every 10min NO HEADER ROW
...
while (curr.year() < 2100) {  //((milli - milli2) < 21600000) { //REPLACE while LOOP WITH TIME-BASED SETPOINT
    curr = rtc.now();
    tensincemidnight = ((curr.hour()*60)+(curr.minute()))/10; // this tells us which row of csv file to read
    Serial.print(F("10-min intervals since midnight = row: ")); //CSV file starts at zero
...
//  CSVfile gotoLine command to navigate to appropriate setpoints
    setFile.open("tig_cycle.csv", O_RDWR | O_CREAT);
  ...
  //begin scenarios for pHset and DOset vs current reading - pulse gases appropriately
    if (getpH() > phLower && getpH() < phUpper && getDOmgL() > doLower && getDOmgL() < doUpper) {
...
//      writeToSD();
    ...
    else if (getpH() > phLower && getpH() < phUpper && getDOmgL() > doUpper) {
...
//      writeToSD();
    ...
    else if (getpH() > phLower && getpH() < phUpper && getDOmgL() < doLower) {
      digitalWrite(N2valve, LOW);
      digitalWrite(CO2valve, LOW);
...
//      writeToSD();
    ...
    else if (getpH() > phUpper && getDOmgL() > doUpper) {
      digitalWrite(N2valve, LOW);
      digitalWrite(CO2valve, HIGH);
      digitalWrite(O2valve, LOW);
      delay(100);
...
//      writeToSD();
    ...
    else if (getpH() > phUpper && getDOmgL() < doLower) {
      digitalWrite(N2valve, LOW);
      digitalWrite(CO2valve, HIGH);
      digitalWrite(O2valve, LOW);
      delay(100);
...
//      writeToSD();
    ...
    else if (getpH() < phLower && getDOmgL() < doLower) {
      digitalWrite(N2valve, HIGH);
      digitalWrite(CO2valve, LOW);
      digitalWrite(O2valve, HIGH);
      delay(450);
...
//      writeToSD();
    ...
    else if (getpH() < phLower && getDOmgL() > doUpper) {
      digitalWrite(N2valve, HIGH);
      digitalWrite(CO2valve, LOW);
      digitalWrite(O2valve, LOW);
      delay(450);
