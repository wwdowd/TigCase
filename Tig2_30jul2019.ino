...
//Before running, use Tig_EEPROM_serial.ino to write serial number to EEPROM memory
//Before running, enter DO/pH resistor values on lines 27-28
...
/*  GLOBAL VARIABLES  */
double pHresist = 220.2;  //must be accurately read for each resistor board by board
double DOresist = 220.4;
...
//      }
  ...
  //begin scenarios for pHset and DOset vs current reading - pulse gases appropriately
    if (phMeas > phLower && phMeas < phUpper && doMeas > doLower && doMeas < doUpper) {  //scenario 1 pH = / DO =
...
//      
    ...
    else if (phMeas > phLower && phMeas < phUpper && doMeas > doUpper) { //scenario 2 pH = / DO +
...
//      Serial.println(" ");
    ...
    else if (phMeas > phLower && phMeas < phUpper && doMeas < doLower) { //scenario 3 ph = / DO -
      digitalWrite(N2valve, LOW);
      digitalWrite(CO2valve, LOW);
...
//      Serial.println(" ");
    ...
    else if (phMeas > phUpper && doMeas > doUpper) { //scenario 4 pH + / DO + 
      digitalWrite(N2valve, LOW);
...
//      Serial.println(" ");
    ...
    else if (phMeas > phUpper && doMeas < doLower) {  //scenario 5 pH + / DO -
      digitalWrite(N2valve, LOW);
      digitalWrite(CO2valve, HIGH);
      digitalWrite(O2valve, HIGH);
      delay(10);
...
//      Serial.println(" ");
    ...
    else if (phMeas > phUpper && doMeas > doLower && doMeas < doUpper) {  //scenario 6 pH + / DO =
      digitalWrite(N2valve, LOW);
      digitalWrite(CO2valve, HIGH);
      digitalWrite(O2valve, LOW);
      delay(10);
...
//      Serial.println(" ");
    ...
    else if (phMeas < phLower && doMeas < doLower) { //scenario 7 pH - / DO - 
      digitalWrite(N2valve, HIGH);
      digitalWrite(CO2valve, LOW);
      digitalWrite(O2valve, HIGH);
      delay(200*phMult);
...
//      Serial.println(" ");
    ...
    else if (phMeas < phLower && doMeas > doUpper) { //scenario 8 pH - / DO + 
      digitalWrite(N2valve, HIGH);
      digitalWrite(CO2valve, LOW);
      digitalWrite(O2valve, LOW);
      delay(200*phMult);
...
//      Serial.println(" ");
    ...
    else if (phMeas < phLower && doMeas > doLower && doMeas < doUpper) {  //scenario 9 pH - / DO =
      digitalWrite(N2valve, HIGH);
      digitalWrite(CO2valve, LOW);
      digitalWrite(O2valve, LOW);
      delay(200*phMult);
...
//   Serial.println(pHRaw);
   pHtemp = (pHRaw)*187.5/1000/pHresist;  //187.5 nominally
   pHVolt = (pHtemp*.5)+1; //(pHtemp+2)/2; // use slope of mA output vs pH (3-20mA for pH 3-11)
   if ((pHVolt) < 3) { //.1875/1000) < 0.5) { //if ADC reading < 0.5V (lowest value should be 0.88V)
    pHVolt = 0;
   }
   //else {pHVolt = (pHRaw*.1875/1000)*2.2727 + 1;//+2390.8)/3135.7; //with 1x gain 1 bit = 0.125mV; this conversion empirical
   else {pHVolt = pHVolt;          //GAIN_TWOTHIRDS 1 bit =.1875 mV https://learn.adafruit.com/adafruit-4-channel-adc-breakouts/arduino-code
   }
   return pHVolt;
}
...
//  Serial.print("DORaw = "); 
//  Serial.println(DORaw);
//  Serial.println(DORaw+1);
  DOtemp = (DORaw)*187.5/1000/DOresist;  // was +1 wd 24jun2019 // = I in Ohm's law equation
  DOmgLMeas = (DOtemp - 3.991996)/.8004002;
  if (DOmgLMeas < 0.01) { //if ADC reading < 0.5V (lowest value should be 0.88V)
    DOmgLMeas = 0;
  }
  else {DOmgLMeas = (DOmgLMeas); //*7.0994 - 6.2375; //convert uMol to mg/L * 0.0319988; CHECK THIS!!!
  }
  return DOmgLMeas;  // 0.01 mg/L = 0.88V; 25 mg/L = 4.4V
  Serial.println(DOmgLMeas);
}
